...
class GeetestSolver:
    ...
    async def _detect_gap_canvas(
        ...
        — this is the gap position.

        Compares ALL rows pixel-by-pixel, column-wise, finds the widest
        contiguous region where ≥10% of rows differ by >30 (total RGB diff),
        and returns the CENTER of that run.

        Returns column position in canvas coordinates, or None if detection fails
        ...
        try:
            result = await frame.evaluate("""() => {
                ...
                const ctx_full = full.getContext('2d');
                if (!ctx_bg || !ctx_full) return null;
                const w = Math.min(bg.width, full.width);
                const h = Math.min(bg.height, full.height);
                if (w < 10 || h < 10) return null;
                const d_bg = ctx_bg.getImageData(0, 0, w, h).data;
                const d_full = ctx_full.getImageData(0, 0, w, h).data;
                // Compare every row, column by column
                // For each column: what fraction of rows differ?
                const diffs = [];
                for (let x = 0; x < w; x++) {
                    let differing = 0;
                    for (let y = 0; y < h; y++) {
                        const i = (y * w + x) * 4;
                        const diff = Math.abs(d_bg[i] - d_full[i])
                                   + Math.abs(d_bg[i+1] - d_full[i+1])
                                   + Math.abs(d_bg[i+2] - d_full[i+2]);
                        if (diff > 30) differing++;
                    }
                    diffs.push(differing / h);
                }
                // Find the widest contiguous region where >10% of rows differ
                let best_start = -1, best_end = -1, best_width = 0;
                let cur_start = -1, cur_end = -1;
                for (let x = 0; x < w; x++) {
                    if (diffs[x] > 0.10) {
                        if (cur_start === -1) cur_start = x;
                        cur_end = x;
                    } else if (cur_start !== -1) {
                        const width = cur_end - cur_start + 1;
                        if (width > best_width) {
                            best_width = width;
                ...
                // Return center of the widest gap region
                if (best_start >= 0) {
                    return Math.round((best_start + best_end) / 2);
                }
                return null;
            }""")
            if result is not None and isinstance(result, (int, float)) and result > 0:
                log.info("[CANVAS] Gap at column %d (full-row bg vs fullbg comparison)", result)
                return int(result)
        except Exception as e:
